...
test('an editable nickname never grants presenter rights to token-admitted guests', () => {
    assert.equal(isNamedPresenter({ bodrikTokenAuthenticated: true, peer_info: { peer_name: 'Host' } }, ['Host']), false);
    assert.equal(isNamedPresenter({ bodrikTokenAuthenticated: true, peer_info: { peer_name: 'Guest' } }, ['Host']), false);
});
