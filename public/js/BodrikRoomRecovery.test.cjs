...
test('leaving suppresses signaling and media recovery until exit cleanup', async () => {
    const { methods, events } = recoveryMethods(async () => { throw new Error('must not rejoin'); });
    let acknowledge;
    const client = {
        socket: {
        ...
        consumerTransport: { id: 'consumer', close() { events.push('consumer closed'); } },
        producerTransport: { id: 'producer', close() { events.push('producer closed'); } },
        closeReconnectAlert() { events.push('alert closed'); },
        stopConsumerReconcile() {},
        handleDisconnect() { throw new Error('must not display disconnection'); },
        showReconnectAlert() { throw new Error('must not display recovery'); },
        handleReconnectAttempt() { throw new Error('must not retry'); },
        event() { events.push('exit event'); },
    };
...
test('leaving after signaling loss cleans up without an exit request', () => {
    ...
    const client = {
        socket: { connected: false, request() { throw new Error('must not request'); }, off() {}, io: { off() {} } },
        producerTransport: { close() { events.push('producer closed'); } },
        closeReconnectAlert() {}, stopConsumerReconcile() {}, event() { events.push('exit event'); },
    };
    methods.exit.call(client);
...
test('exit request failure cleans up without a reconnect banner', async () => {
    ...
    const client = {
        socket: {
            connected: true, request() { return Promise.reject(new Error('offline')); },
            off() {}, io: { off() {} },
        },
        producerTransport: { close() { events.push('producer closed'); } },
        closeReconnectAlert() { events.push('alert closed'); },
        stopConsumerReconcile() {}, event() { events.push('exit event'); },
    };
    methods.exit.call(client);
    await new Promise(setImmediate);
